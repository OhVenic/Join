...
function showContactDetails(contactId) {
  ...
  badge.textContent =
    contactId.split("-")[0].charAt(0).toUpperCase() +
      contactId.split("-")[1]?.charAt(0).toUpperCase() || "";
...
function createContactCard(contactId, contact) {
  ...
  contactCard.innerHTML = `
    <div class="user-badge ${contact.name
      .charAt(0)
      .toLowerCase()}">${contact.name.charAt(0).toUpperCase()}${
...
function addContact(event) {
  ...
  let letterSection = document.querySelector(
    `.letter-section h2[data-letter="${firstLetter}"]`
  )?.parentElement;
  if (!letterSection) {
...
function updateContactCard(contact) {
  const contactCard = document.querySelector(
    `.contact-card[onclick="showContactDetails('${currentContactId}')"]`
  );
...
function removeContactFromTasks() {
  Object.values(tasks).forEach((task) => {
    task.assigned = task.assigned.filter(
      (person) =>
        person !==
        currentContactId.split("-")[0].toUpperCase() +
          currentContactId.split("-")[1]?.charAt(0).toUpperCase()
    );
